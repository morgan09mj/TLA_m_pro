...
namespace fa_3
{
    ...
    public class ConvertJsonToFA
    {
        ...
        public class FAAcceptance
    {
        public static bool IsAccepted(string input, DFA automata)
        {
            //temp state to move through automata by each character 
            NewState temp_state = automata.InitialState;

            //move through states by the order of input string characters 
            for (int i = 0; i < input.Length; i++)
                temp_state = temp_state.transitions[input[i].ToString()];

            //input string is consumed check whether the state we're in is final or non-final
            if (automata.FinalStates.Contains(temp_state))
                return true;
            else
                return false;
        }
        public static void Main(string[] args)
        {
            ...
            FA fa = JsonSerializer.Deserialize<FA>(json_text);
            var states = Regex.Replace(fa.states, @"[{}']", "").Split(",").ToList();
            var _input_symbols = Regex.Replace(fa.input_symbols, @"[{}']", "").Split(",").ToList();
            var final_states = Regex.Replace(fa.final_states, @"[{}']", "").Split(",").ToList();
            var transitions = fa.transitions.First().Value;
            DFA dfa = null;
            if (transitions.First().Value.Contains('{'))
            {
                NFA nfa = null;
                var _states = states.Select(x => new State(x)).ToList();
                List<State> _final_states = new List<State>();
                for (int i = 0; i < final_states.Count(); i++)
                    _final_states.Add(_states.Where(x => x.state_ID == final_states[i]).First());
                var _initial_state = _states.Where(x => x.state_ID == fa.initial_state).First();
            ...
            {
                var _states = _states.Select(x => new NewState(x)).ToList();
                List<NewState> _final_states = new List<NewState>();
                for (int i = 0; i < final_states.Count(); i++)
                    _final_states.Add(_states.Where(x => x.state_ID == final_states[i]).First());
                var _initial_state = _states.Where(x => x.state_ID == fa.initial_state).First();
